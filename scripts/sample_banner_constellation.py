...
with sync_playwright() as p:
    ...
    points = page.evaluate('''async (source) => {
      ...
      for (let y = 0; y < 500; y += 3) {
        for (let x = 0; x < 500; x += 3) {
          let best = 40, point;
          for (let dy = 0; dy < 3 && y + dy < 500; dy++) {
            for (let dx = 0; dx < 3 && x + dx < 500; dx++) {
    ...
    browser.close()
(ROOT / 'assets/constellations/banner-stars.json').write_text(json.dumps(points, separators=(',', ':')) + '\n')
