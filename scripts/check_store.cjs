...
class Element {
  constructor() { this.children = []; this.value = ''; this.hidden = false; }
...
async function setup({ configured = true, fail = false, warnings = false } = {}) {
  ...
  const context = {
    ...
    fetch: async (url, options) => {
      ...
      const data = request.query.includes('mutation Checkout') ? { cartCreate: {
        userErrors: [], warnings: warnings ? [{ message: 'Stock has changed.' }] : [],
        cart: { checkoutUrl: 'https://zodiac-test.myshopify.com/checkouts/test',
          lines: { nodes: request.variables.input.lines.map(line => ({ quantity: line.quantity, merchandise: { id: line.merchandiseId } })) } }
      } } : { products: { pageInfo: { hasNextPage: false, endCursor: 'end' }, nodes: [{
        id: 'gid://shopify/Product/1', title: 'Team jersey', description: 'Test product', featuredImage: null,
        variants: { pageInfo: { hasNextPage: false }, nodes: [
          { id: 'gid://shopify/ProductVariant/1', title: 'S', availableForSale: false, price: { amount: '30', currencyCode: 'USD' } },
          { id: 'gid://shopify/ProductVariant/2', title: 'M', availableForSale: true, price: { amount: '35', currencyCode: 'USD' } }
        ] }
      }] } };
...
(async () => {
  ...
  for (const warnings of [false, true]) {
    ...
    const select = details.children[4], add = details.children[5];
  ...
  console.log('Store checks passed: launch state, API failure, variants, bag controls, checkout payload, and inventory warnings.');
