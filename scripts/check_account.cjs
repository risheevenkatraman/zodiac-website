...
function setup() {
  ...
  const location = {
    search: '',
    pathname: '/zodiac/account.html',
    assign: (url) => redirects.push(url),
  };
  const config = {
    enabled: true,
    shopDomain: 'zodiac.myshopify.com',
    clientId: 'public-client',
    customerApiUrl: 'https://customer.test/graphql',
    rewardsApiUrl: 'https://api.test',
  };
  const context = {
    window: {},
    document: { currentScript: { src: 'https://zodiac.test/zodiac/js/account-auth.js' } },
    URL,
    URLSearchParams,
    Uint8Array,
    TextEncoder,
    ...
    fetch: async (url, options) => {
      ...
      if (url.endsWith('data/account.json')) value = config;
        ...
        value = {
          data: { customer: { id: 'gid://shopify/Customer/10', displayName: 'Supporter' } },
        };
...
(async () => {
  ...
  assert.equal(
    ...
    Buffer.from(
      await webcrypto.subtle.digest('SHA-256', new TextEncoder().encode(flow.verifier)),
    ).toString('base64url'),
  );
  ...
  assert.equal(
    login.calls.some((call) => call.url.endsWith('/token')),
    false,
  );
  ...
  callback.storage.set(
    ...
    JSON.stringify({
      ...
      verifier: 'verifier',
      created: Date.now(),
      redirect: 'https://zodiac.test/zodiac/account.html',
    }),
  );
  ...
  expired.storage.set(
    'zodiac.customer',
    JSON.stringify({ access_token: 'expired', expires: Date.now() - 1 }),
  );
  ...
  console.log(
    'Account checks passed: PKCE, callback state, tab session, expiry, authenticated requests, redemption, and logout.',
  );
})().catch((error) => {
  console.error(error);
  process.exitCode = 1;
