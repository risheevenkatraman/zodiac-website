...
function parseAnnouncement(announcement) {
  ...
  const field = (key) =>
    typeof announcement[key] === 'string' && announcement[key].trim()
      ? announcement[key].trim()
      : fallbackAnnouncement[key];
  return {
    title: field('title'),
...
function renderAnnouncement(announcement) {
  ...
  try {
    ...
    image.src = ['https:', 'http:', 'file:'].includes(url.protocol)
      ? url.href
      : fallbackAnnouncement.image;
  } catch {
    image.src = fallbackAnnouncement.image;
  }
