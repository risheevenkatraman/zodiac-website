...
window.ZodiacAccount = (() => {
  ...
  const sessionKey = 'zodiac.customer', flowKey = 'zodiac.login';
  ...
  const https = value => {
    ...
    if (url.protocol !== 'https:' || url.username || url.password) throw new Error('Invalid account configuration.');
    return url.href;
  };
  ...
  async function endpoints() {
    ...
    if (!/^[a-z0-9][a-z0-9-]*\.myshopify\.com$/.test(c.shopDomain) || !c.clientId) throw new Error('Invalid account configuration.');
    if (!discovery) discovery = await json(`https://${c.shopDomain}/.well-known/openid-configuration`);
    return discovery;
  }
  ...
  function session() {
    ...
    } catch { /* Missing or expired session. */ }
    sessionStorage.removeItem(sessionKey);
    return null;
  }
  async function login() {
    const c = await config(), d = await endpoints();
    const verifier = random(), state = random(), nonce = random();
    const redirect = new URL('account.html', root).href;
    const challenge = encode(new Uint8Array(await crypto.subtle.digest('SHA-256', new TextEncoder().encode(verifier))));
    sessionStorage.setItem(flowKey, JSON.stringify({ verifier, state, nonce, redirect, created: Date.now() }));
    const url = new URL(https(d.authorization_endpoint));
    url.search = new URLSearchParams({ client_id: c.clientId, response_type: 'code', redirect_uri: redirect,
      scope: 'openid email customer-account-api:full', state, nonce, code_challenge: challenge, code_challenge_method: 'S256' });
  ...
  async function callback() {
    ...
    if (!flow || params.get('state') !== flow.state || Date.now() - flow.created > 600000) throw new Error('Sign-in expired. Please sign in again.');
    if (params.has('error')) throw new Error('Sign-in was cancelled. You can try again.');
    const c = await config(), d = await endpoints();
    const token = await json(https(d.token_endpoint), { method: 'POST',
      headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
      body: new URLSearchParams({ grant_type: 'authorization_code', client_id: c.clientId,
        code: params.get('code'), redirect_uri: flow.redirect, code_verifier: flow.verifier }) });
    // Identity is checked against Shopify's API; decoded JWT claims are never used as identity.
    const payload = JSON.parse(atob(token.id_token.split('.')[1].replaceAll('-', '+').replaceAll('_', '/')));
    if (payload.nonce !== flow.nonce || !token.access_token || !(token.expires_in > 0)) throw new Error('Could not verify sign-in. Please try again.');
    sessionStorage.setItem(sessionKey, JSON.stringify({ access_token: token.access_token,
      id_token: token.id_token, expires: Date.now() + Number(token.expires_in) * 1000 }));
  }
  async function customer() {
    const c = await config(), s = session();
    if (!s) return null;
    const result = await json(https(c.customerApiUrl), { method: 'POST',
      headers: { 'Content-Type': 'application/json', Authorization: s.access_token },
      body: JSON.stringify({ query: '{ customer { id displayName emailAddress { emailAddress } } }' }) });
  ...
  async function rewards() {
    ...
    return json(https(c.rewardsApiUrl).replace(/\/$/, '') + '/me/stars', { headers: { Authorization: `Bearer ${s.access_token}` } });
  }
  async function redeem(requestId) {
    ...
    const response = await fetch(https(c.rewardsApiUrl).replace(/\/$/, '') + '/me/redeem', {
      ...
      body: JSON.stringify({ requestId }), signal: AbortSignal.timeout(20000)
    });
