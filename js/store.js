(() => {
  ...
  function saveBag() {
    try {
      ...
      /* Shopping also works when browser storage is disabled. */
    }
  }
  function restoreBag() {
    try {
      ...
      if (
        !draft ||
        draft.domain !== config.domain ||
        Date.now() - draft.saved > 86400000 ||
        !Array.isArray(draft.lines) ||
        draft.lines.length > 250
      )
        return;
      const restored = [],
        seen = new Set();
      for (const line of draft.lines) {
        ...
        if (
          !/^gid:\/\/shopify\/ProductVariant\/[0-9]+$/.test(line.merchandiseId) ||
          seen.has(line.merchandiseId) ||
          ...
          Number(v.price.amount) < 0 ||
          !/^[A-Z]{3}$/.test(v.price.currencyCode)
        )
          return;
      ...
      /* Ignore an expired or malformed bag. Shopify validates checkout. */
    }
  }
  const money = (value) =>
  ...
  async function api(query, variables = {}) {
    ...
    if (payload.errors?.length || !payload.data)
      throw new Error('The store could not complete this request. Please try again.');
    return payload.data;
  }
  function drawCart() {
    ...
    for (const line of cart) {
      ...
      row.append(
        node('strong', variant.productTitle),
        node('p', `${variant.title} · ${money(variant.price)}`),
      );
      ...
      for (const [label, delta] of [
        ...
        button.setAttribute(
          'aria-label',
          `${delta === 1 ? 'Increase' : 'Decrease'} quantity of ${variant.productTitle}, ${variant.title}`,
        );
        button.onclick = () => {
          line.quantity += delta;
      ...
      remove.onclick = () => {
        cart = cart.filter((item) => item !== line);
        drawCart();
        el('cart-status').textContent = 'Item removed.';
      };
    ...
    if (!cart.length)
      el('cart-lines').append(
        node('p', 'Your bag is empty. Find your next favorite in the collection.'),
      );
    el('cart-count').textContent = `(${count})`;
    el('cart-total').textContent = currency
      ? money({ amount: total, currencyCode: currency })
      : '—';
  ...
  function productCard(product) {
    ...
    details.append(
      ...
      node('p', product.description, 'product-description'),
    );
    const price = node('p');
    ...
    add.onclick = () => {
      ...
      if (existing?.quantity >= 99) {
        el('cart-status').textContent = 'Maximum quantity is 99 per option.';
        return;
      }
      if (cart.length >= 250 && !existing) {
        el('cart-status').textContent = 'Your bag is full. Please check out before adding more.';
        return;
      }
  ...
  async function loadProducts() {
    ...
    try {
      ...
      for (const product of data.products.nodes) {
        while (product.variants.pageInfo.hasNextPage) {
          const more = await api(
            `query Options($id: ID!, $after: String!) {
      ...
      el('store-status').textContent = el('products').children.length
        ? 'Choose your gear and make it yours.'
  ...
  el('checkout').onclick = async () => {
    ...
    try {
      const data = await api(
        `mutation Checkout($input: CartInput!) {
      ...
      }`,
        {
          input: {
            lines: cart,
            ...(window.ZodiacAccount?.session()
              ? {
                  buyerIdentity: {
                    customerAccessToken: window.ZodiacAccount.session().access_token,
                  },
                }
              : {}),
          },
      ...
      if (
        !actual ||
        actual.length !== cart.length ||
        cart.some(
          (line) =>
            !actual.some(
              (item) =>
                item.merchandise.id === line.merchandiseId && item.quantity === line.quantity,
            ),
        )
      ) {
      ...
      el('cart-status').textContent =
        error.name === 'TimeoutError' ? 'Checkout took too long. Please try again.' : error.message;
    } finally {
      busy = false;
  ...
  async function init() {
    try {
      const response = await fetch(assetRoot + 'data/store.json', {
        cache: 'no-store',
      ...
      if (!settings.domain || !settings.publicStorefrontAccessToken) {
        el('store-status').textContent =
          'Our first collection is on its way. Shopping will open here when the store launches.';
        el('payment-note').textContent = 'Checkout opens when the collection launches.';
        return;
      }
      if (
        !/^[a-z0-9][a-z0-9-]*\.myshopify\.com$/.test(settings.domain) ||
        !/^\d{4}-(01|04|07|10)$/.test(settings.apiVersion)
      )
        throw new Error('Invalid configuration');
      config = settings;
      restoreBag();
      if (config.shopPayEnabled === true)
        el('payment-note').textContent =
          'Secure Shopify checkout. Choose Shop Pay at checkout where available.';
      await loadProducts();
    } catch {
